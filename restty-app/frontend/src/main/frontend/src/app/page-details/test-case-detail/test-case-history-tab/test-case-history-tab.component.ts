...
import { Component, OnInit, ViewEncapsulation, OnChanges, Input, ChangeDetectorRef, NgZone, SimpleChanges } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
import { FilterConfig, SortConfig, ToolbarConfig, SortField, PaginationConfig, EmptyStateConfig, TableConfig,
  FilterType, FilterField, Filter, FilterEvent, PaginationEvent, SortEvent, TableEvent } from 'patternfly-ng';
import { TimeAgoPipe } from 'time-ago-pipe';

@Component({
  selector: 'app-test-case-history-tab',
  templateUrl: './test-case-history-tab.component.html',
  styleUrls: ['./test-case-history-tab.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class TestCaseHistoryTabComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    this.route.params.subscribe(pathVariable => {
      this.testCaseId = pathVariable.testCaseId;
    });
  }

  ngOnInit() {
    this.initTable();
    this.logService.findAllByTestCase(this.testCaseId).subscribe(logs => {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.update.currentValue && this.allRows !== undefined) {
      this.logService.findRecentLogByTestCase(this.testCaseId).subscribe(log => {
        this.allRows.push({
          run: new TimeAgoPipe(this.ref, this.ngZone).transform(log.run),
          runVal: log.run,
          responseStatus: log.responseStatus,
  ...
  initConfigurations() {
    ...
    this.emptyStateConfig = {
      ...
      info: 'No logs are available. Start by runnin the test case.'
    } as EmptyStateConfig;
