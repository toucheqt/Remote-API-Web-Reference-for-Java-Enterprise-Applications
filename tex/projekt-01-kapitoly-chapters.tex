...
the~syntax for~structured, generic, and~\uv{object-oriented} programming, Java
was built almost exclusively as~an~\uv{object-oriented} language. All code is
...
the~words \uv{Hello World!} if executed.
...
\begin{enumerate}
  ...
  of~the~class name and~the~\uv{.java} suffix. If the~file name and~the~class
...
good programming practices by~enabling a \uv{POJO\footnote{In~software
engineering, a~Plain Old Java Object (POJO) is an~ordinary Java object, not
bound by~any special restriction and~not requiring any class path.}-based}
programming model.
...
through a~class constructor. Thus the~flow of~control has been \uv{inverted}
...
The~other key~component of~Spring framework is the~Aspect oriented programming
(AOP) framework. \uv{Aspect-Oriented} Programming entails breaking down program
logic into distinct parts called \uv{so-called} concerns. The~functions that
span multiple points of~an~application are called \uv{cross-cutting} concerns
...
helps decouple \uv{cross-cutting} concerns from~the~objects that they affect.
...
the~\uv{@GeneratedValue} annotation, which takes two parameters strategy
and~generator.
