...
The~\uv{User-Agent} header contains information about the~type of~a~client which
...
In~the~listing~\ref{GETExampleResponse}, can be seen an~example
...
To~avoid confusion, let us have an~example. Suppose a~database
...
It should allow creating test cases that can combine various endpoints and~other
...
the~\uv{tree-like} structure was used for~the~presentation of~the~API.
...
As~can be seen in~figure~\ref{figAPI} by~default only \uv{top-level} nodes
